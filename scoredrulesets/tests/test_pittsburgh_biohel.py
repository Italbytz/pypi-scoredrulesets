"""Tests for BioHEL-inspired enhancements in PittsburghRuleSetClassifier.
...
class TestTokenCompetition:
    ...
    def test_zero_weight_matches_baseline(self):
        ...
        X, y = _iris()
        base = PittsburghRuleSetClassifier(
            max_rules=3, beam_width=4, max_iterations=6, random_state=0,
        )
        base.fit(X, y)

        tc = PittsburghRuleSetClassifier(
...
class TestRuleCompaction:
    ...
    def test_compaction_preserves_accuracy(self):
        ...
        X, y = _iris()
        no_compact = PittsburghRuleSetClassifier(
        ...
        compact = PittsburghRuleSetClassifier(
            enable_compaction=True,
            max_rules=5, random_state=0,
        )
        compact.fit(X, y)
...
class TestWindowing:
    ...
    def test_full_window_is_default(self):
        X, y = _iris()
        clf = PittsburghRuleSetClassifier(max_rules=3, random_state=0)
        clf.fit(X, y)
