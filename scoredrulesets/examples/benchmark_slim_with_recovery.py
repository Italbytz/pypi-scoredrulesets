"""Resume-Wrapper fuer den schlanken Benchmark.

Gleiche Funktionalitaet wie ``benchmark_full_report_with_recovery.py``,
aber fuer den Slim-Benchmark (maximale Typ-Diskriminierung).

Aufruf:
    python examples/benchmark_slim_with_recovery.py
    python examples/benchmark_slim_with_recovery.py --max-attempts 5
    python examples/benchmark_slim_with_recovery.py --repeats 10
"""
...
import benchmark_slim_discriminability as slim_report
...
def run_with_resume(
    *,
    ...
    checkpoint_path: str = "benchmarks/checkpoint_slim.jsonl",
    extra_kwargs: dict | None = None,
) -> int:
    """Fuehrt den Slim-Benchmark mit automatischem Resume aus.
    ...
    for attempt in range(1, max_attempts + 1):
        resolved_log = log_file or slim_report._default_log_path()
        print(
            f"\n{'=' * 70}\n"
        ...
        with slim_report._maybe_tee_to_file(resolved_log):
            try:
                slim_report.main(log_file=resolved_log, **kwargs)
                print(
                    f"\n[RESUME] Slim-Benchmark erfolgreich abgeschlossen "
                    f"(Versuch {attempt}).",
                    flush=True,
                )
                return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Checkpoint-Datei (default: benchmarks/checkpoint_slim.jsonl).",
    )
    # Durchreichbare slim_report-Argumente
