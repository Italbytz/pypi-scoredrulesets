...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Erzeugt einen kombinierten Meta-Report aus zwei Benchmark-Laeufen")
    ...
    parser.add_argument("--title", default="Combined Benchmark Meta Report", type=str)
...
def main() -> None:
    ...
    comparison_rows = compare_benchmark_runs(
        core_results,
        compare_results,
    ...
    report = format_benchmark_meta_report_markdown(
        ...
        core_artifacts={
            "aggregated_json": str(core_path),
        },
        compare_artifacts={
            "aggregated_json": str(compare_path),
        },
    ...
    print(f"Markdown meta report: {output_path}")

    if output_html_path is not None:
        html_report = format_benchmark_meta_report_html(
            core_results,
            compare_results,
            comparison_rows,
            title=args.title,
        ...
        print(f"HTML meta report: {output_html_path}")
