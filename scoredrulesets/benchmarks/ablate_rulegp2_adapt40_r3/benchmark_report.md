...
- **warning_runs**: `0`
- **warning_models**: `0`
- **top_1_model**: `uci_wine / ruleGP2` (f1=0.9154, rules=4.0000, fit_s=7.2435)

## Configuration

- **datasets**: `sklearn_wine, uci_heart_disease, synth_checkerboard_4x4`
- **estimators**: `ruleGP2, ruleGP2 (adapt=40)`
- **repeats**: `3`
...
- **best_model**: `ruleGP2` (f1=0.9154, rules=4.0000, fit_s=7.2435)
- **smallest_model**: `ruleGP2` (rules=4.0000, atoms=3.0000, f1=0.9154)
- **fastest_model**: `ruleGP2` (fit_s=7.2435, f1=0.9154, rules=4.0000)

### uci_heart_disease

- **best_model**: `ruleGP2 (adapt=40)` (f1=0.7397, rules=4.3333, fit_s=14.6843)
- **smallest_model**: `ruleGP2` (rules=3.6667, atoms=3.3333, f1=0.7378)
- **fastest_model**: `ruleGP2` (fit_s=10.1407, f1=0.7378, rules=3.6667)

### synth_checkerboard_4x4

- **best_model**: `ruleGP2 (adapt=40)` (f1=0.5045, rules=4.3333, fit_s=24.8471)
- **smallest_model**: `ruleGP2` (rules=3.0000, atoms=2.3333, f1=0.4990)
- **fastest_model**: `ruleGP2` (fit_s=11.6962, f1=0.4990, rules=3.0000)
...
| estimator | f1_mean | atoms | rules |
| --- | ---: | ---: | ---: |
| ruleGP2 (adapt=40) | 0.7397 | 3.6667 | 4.3333 |
...
| estimator | f1_mean | atoms | rules |
| --- | ---: | ---: | ---: |
| ruleGP2 (adapt=40) | 0.5045 | 5.0000 | 4.3333 |
...
| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | uci_wine | ruleGP2 | 3 | 0.9154 | 0.0454 | 7.2435 | 4.0000 | 3.0000 |
| 2 | uci_wine | ruleGP2 (adapt=40) | 3 | 0.8985 | 0.0055 | 11.3445 | 5.0000 | 4.3333 |
| 3 | uci_heart_disease | ruleGP2 (adapt=40) | 3 | 0.7397 | 0.0126 | 14.6843 | 4.3333 | 3.6667 |
| 4 | uci_heart_disease | ruleGP2 | 3 | 0.7378 | 0.0161 | 10.1407 | 3.6667 | 3.3333 |
| 5 | synth_checkerboard_4x4 | ruleGP2 (adapt=40) | 3 | 0.5045 | 0.0030 | 24.8471 | 4.3333 | 5.0000 |
| 6 | synth_checkerboard_4x4 | ruleGP2 | 3 | 0.4990 | 0.0255 | 11.6962 | 3.0000 | 2.3333 |
...
| rank | dataset | estimator | repeats | f1_mean | f1_err | fit_s | rules | atoms |
| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | uci_heart_disease | ruleGP2 (adapt=40) | 3 | 0.7397 | 0.0126 | 14.6843 | 4.3333 | 3.6667 |
| 2 | uci_heart_disease | ruleGP2 | 3 | 0.7378 | 0.0161 | 10.1407 | 3.6667 | 3.3333 |
...
| rank | dataset | estimator | repeats | f1_mean | f1_err | fit_s | rules | atoms |
| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | synth_checkerboard_4x4 | ruleGP2 (adapt=40) | 3 | 0.5045 | 0.0030 | 24.8471 | 4.3333 | 5.0000 |
| 2 | synth_checkerboard_4x4 | ruleGP2 | 3 | 0.4990 | 0.0255 | 11.6962 | 3.0000 | 2.3333 |
